...
export const getAllIndexes = (label: string, substr: string): number[] => {
    let first = 0;
    const result: number[] = [];
    while (true) {
...
export const replaceRstarColorsWith = (label: string, color: string): string => {
    if (!label.includes("~")) return label;

    const findIndexes = getAllIndexes(label, "~");
    return findIndexes.reverse().filter((_, i) => i % 2 === 0).reduce((currentLabel, index, i) => {
