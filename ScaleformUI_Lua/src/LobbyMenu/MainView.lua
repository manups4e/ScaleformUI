    ...
    Citizen.CreateThread(function()
        ...
        while it <= #items do
            ...
            elseif SubType == "UIMenuProgressItem" then
                ...
                ScaleformUI.Scaleforms._pauseMenu._lobby:CallFunction("SET_RIGHT_LABEL", false, it - 1, item:RightLabel())
                if item._rightBadge ~= BadgeStyle.NONE then
                    ScaleformUI.Scaleforms._pauseMenu._lobby:CallFunction("SET_RIGHT_BADGE", false, it - 1, item._rightBadge)
                end
            end
            ...
            if (SubType == "UIMenuItem" and item._leftBadge ~= BadgeStyle.NONE) or (SubType ~= "UIMenuItem" and item.Base._leftBadge ~= BadgeStyle.NONE) then
                if SubType ~= "UIMenuItem" then
                    ScaleformUI.Scaleforms._pauseMenu._lobby:CallFunction("SET_LEFT_BADGE", false, it - 1, item.Base._leftBadge)
                else
                    ScaleformUI.Scaleforms._pauseMenu._lobby:CallFunction("SET_LEFT_BADGE", false, it - 1, item._leftBadge)
                end
            end
