...
AddEventHandler("ScaleformUI_Lua:lobbymenu:MapPanel", function(data)
		...
		if minimapLobbyEnabled then
			...
			for k,v in pairs(data) do
				...
				elseif k == #data then
					...
					table.insert(LobbyMenu.Minimap.MinimapRoute.CheckPoints, MinimapRaceCheckpoint.New(271, vector3(v.x, v.y, v.z), blipColor, 0.8))
				end
			end
		else
...
AddEventHandler("ScaleformUI_Lua:lobbymenu:Show", function(canClose, onClose)
	CreateLobbyMenu()
            ...
            if currentColumnId == 2 and DataSet.PlayerList[currentSelectId] and DataSet.PlayerList[currentSelectId].ped then
                ...
                table.insert(settingList, {
                    ...
                    callbackFunction = function()
                        TriggerEvent("ScaleformUI_Lua:playermenu:Hide")
                    end,
                })
                ...
                TriggerEvent("ScaleformUI_Lua:playermenu:SetInfo", DataSet.Info)
                ...
                TriggerEvent("ScaleformUI_Lua:playermenu:SetPlayerList", DataSet.PlayerList[currentSelectId], LobbyMenu.MissionPanel.TextureDict, LobbyMenu.MissionPanel.TextureName)

                TriggerEvent("ScaleformUI_Lua:playermenu:Show", function()
                    TriggerEvent("ScaleformUI_Lua:lobbymenu:Show", true)
                end)
            end
		end
...
RegisterNetEvent("ScaleformUI_Lua:lobbymenu:leaveLobby")
AddEventHandler("ScaleformUI_Lua:lobbymenu:leaveLobby", function()
...
RegisterCommand('slua_setmappanel', function(source, args)
	...
	TriggerEvent("ScaleformUI_Lua:lobbymenu:MapPanel", route)
end, false)
...
RegisterCommand('slua_show', function(source, args)
    ...
    TriggerEvent("ScaleformUI_Lua:lobbymenu:Show", true, function()
